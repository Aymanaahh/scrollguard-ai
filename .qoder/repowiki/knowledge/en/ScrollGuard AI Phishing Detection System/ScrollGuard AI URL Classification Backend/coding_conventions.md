...
- Request schemas are defined as Pydantic `BaseModel` classes (`AnalysisRequest`, `URLBatch`) and used directly as FastAPI route parameters.
...
- Concurrent batch analysis uses `asyncio.gather` over coroutines wrapped by a module-level `asyncio.Semaphore` to cap concurrent LLM calls.